...
async def test_list_books_pagination_edges(test_author_id):
    async for session in app.dependency_overrides[get_session]():
        ...
        for i in range(seed_count):
            await repo.create(BookEntity(id=None, title=f"Seed {i} {uuid4()}", authors=[test_author_id]))
        ...
        expected_last = (page1.meta.total - 1) // page_size + 1 if page1.meta.total > 0 else 1
...
async def test_create_book_success_and_conflict(test_author_id):
    async for session in app.dependency_overrides[get_session]():
        ...
        created = await service.create_book(BookCreateData(title=base_title, authors=[test_author_id]))
        assert created.id is not None
        # Duplicate (case/space variation) should raise ConflictException
        with pytest.raises(ConflictException):
            await service.create_book(BookCreateData(title=base_title.upper(), authors=[test_author_id]))
...
async def test_update_book_success_and_conflict(test_author_id):
    async for session in app.dependency_overrides[get_session]():
        ...
        b1 = await service.create_book(BookCreateData(title=t1, authors=[test_author_id]))
...
async def test_delete_book_success_and_not_found(test_author_id):
    async for session in app.dependency_overrides[get_session]():
        ...
        book = await service.create_book(BookCreateData(title=title, authors=[test_author_id]))
