...
async def test_create_book_validation_and_conflict(test_author_id):
    async for repo, service in _make_service():
        ...
        with pytest.raises(ValueError):
            await service.create_book(BookCreateData(title="   ", authors=[test_author_id]))
        # Authors invalid (contains non-positive)
        with pytest.raises(ValueError):
            await service.create_book(BookCreateData(title="Valid", authors=[0]))
        base = f"Title {uuid4()}"
        b1 = await service.create_book(BookCreateData(title=base, authors=[test_author_id]))
        assert b1.id is not None
        # Duplicate (case + spaces)
        with pytest.raises(ConflictException):
            await service.create_book(BookCreateData(title=f"  {base.upper()}  ", authors=[test_author_id]))
...
async def test_update_book_branches(test_author_id):
    async for repo, service in _make_service():
        ...
        # Seed two books
        b1 = await service.create_book(BookCreateData(title=f"Seed1 {uuid4()}", authors=[test_author_id]))
        b2 = await service.create_book(BookCreateData(title=f"Seed2 {uuid4()}", authors=[test_author_id]))
        # Empty title
        with pytest.raises(ValueError):
        ...
        async for session in app.dependency_overrides[get_session]():
            ...
            author2 = await arepo.create(AuthorCreateData(first_name=f"Temp{uuid4()}", last_name="Author"))
            new_author_id = author2.id
        updated = await service.update_book(b1.id, BookUpdateData(authors=[new_author_id]))
...
async def test_delete_book_paths(test_author_id):
    async for repo, service in _make_service():
        ...
        b = await service.create_book(BookCreateData(title=f"Del {uuid4()}", authors=[test_author_id]))
...
async def test_list_books_pagination_error_and_success(test_author_id):
    async for repo, service in _make_service():
        ...
        # Seed 12 books
        for i in range(12):
            await repo.create(BookCreateData(title=f"P{i}-{uuid4()}", authors=[test_author_id]))
...
async def test_repository_update_delete_edge_returns(test_author_id):
    async for repo, service in _make_service():
        ...
        updated_none = await repo.update(999999, BookEntity(id=999999, title="X", authors=[test_author_id]))
